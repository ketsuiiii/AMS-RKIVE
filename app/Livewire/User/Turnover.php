...
use App\Models\Old\Turnover as ModelTurnover;
...
class Turnover extends Component
{
    ...
    public function store()
    {
        ...
        ModelTurnover::onConnection1()->create([
    ...
    {
        $turnover = ModelTurnover::onConnection1()->findOrFail($id);
        $this->turnoverId = $id;
    ...
    public function update()
    {
        ...
        if ($this->turnoverId) {
            $turnover = ModelTurnover::onConnection1()->findOrFail($this->turnoverId);
            $turnover->update([
                'turnover_info' => $this->turnover_info,
                'turnover_department' => $this->turnover_department,
                'turnover_cost_of_goods_sold' => $this->turnover_cost_of_goods_sold,
    ...
    {
        $result = [] ?: ModelTurnover::onConnection1()->all();
        $search = $this->search;

        if (strlen($search) > 0) {
            $result = ModelTurnover::onConnection1()->where('turnover_info', 'like', '%' . $search . '%')
        ...
        $data = [
            'departments' => Department::onConnection1()->all(),
            'categories' => PlanCategories::onConnection1()->where('category_type', 'T8')->get(),
            'turnover' => $result,
        ];
